...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 5000) api.sendMessage("Bạn cần 50005000 đô để xem ảnh ?",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 5000})
   var callback = () => api.sendMessage({body:`Ảnh Ganyu genshi\nSố Ảnh: ${link.length}\n-50005000 đô !`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
