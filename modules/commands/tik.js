...
module.exports.run = async function({ api, event, args,}) {
  ...
  try {
	const options = {
	  ...
	  else return api.sendMessage({body : "tớ lấy video cho cậu rồi nè, thấy tớ giỏi hongg :3 ✅" , attachment: fs.createReadStream(__dirname + `/cache/${event.threadID}-${event.senderID}.mp4`)}, event.threadID, () => fs.unlinkSync(__dirname + `/cache/${event.threadID}-${event.senderID}.mp4`), event.messageID)
